...
export default function InstallationScheduleScreen({ api, projectId, user, tcUserEmail, tcUserName, onBackToMenu }: Props) {
  ...
  // Open Gantt Timeline in popup window
  const openGanttTimeline = async () => {
    try {
      setMessage('Laadin Gantt graafikut...');
      ...
      for (const item of scheduleItems) {
        ...
        for (const [key, val] of Object.entries(methods)) {
          if (val && val > 0) {
            const k = key as InstallMethodType;
            installByDate[date].resources[k] = Math.max(installByDate[date].resources[k] || 0, val);
          }
        }
      }
...
<head>
  <meta charset="UTF-8">
  <title>Gantt Timeline - ${projectName || 'Projekt'}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h3 { margin: 24px 0 12px; color: #555; font-size: 14px; }
    ...
    .resource-cell { background: #fce4ec; color: #c2185b; }
    .resource-cell.crane { background: #fff8e1; color: #f57c00; }
...
<body>
  <h2>📊 Gantt Timeline - ${projectName || 'Projekt'}</h2>
  ...
  <div class="gantt-container">
    <table>
      <thead>
        <tr>
          ...
          ${sortedDates.map(d => {
            ...
            return `<th class="date-header ${isWeekend ? 'weekend' : ''}">
              ${formatDate(d)}<br><span class="weekday">${wd}</span>
            </th>`;
          }).join('')}
        </tr>
      </thead>
      <tbody>
        ...
        ${Object.entries(deliveryByFactory).map(([factory, info]) => `
          <tr>
            ...
            ${sortedDates.map(d => {
              ...
              if (dayInfo) {
                ...
                return `<td class="delivery-cell ${isMulti ? 'multi' : ''} ${isWeekend ? 'weekend' : ''}">
                  ${dayInfo.count} tk
                  <span class="trucks">${dayInfo.trucks.join(', ')}</span>
                </td>`;
              }
        ...
        <tr>
          <td class="row-header">Paigaldus</td>
          ${sortedDates.map(d => {
            const wd = getWeekday(d);
            const isWeekend = wd === 'L' || wd === 'P';
            const dayInfo = installByDate[d];
            if (dayInfo) {
              return `<td class="install-cell ${isWeekend ? 'weekend' : ''}">${dayInfo.count} detaili</td>`;
            }
            return `<td class="${isWeekend ? 'weekend' : ''}"><span class="empty">-</span></td>`;
          }).join('')}
        </tr>
        ...
        <!-- Resources Section -->
        <tr><td class="row-header section-title" colspan="${sortedDates.length + 1}">👷 RESSURSID (Paigaldus)</td></tr>
        ${(['crane', 'forklift', 'poomtostuk', 'kaartostuk', 'manual'] as InstallMethodType[]).map(method => {
          ...
          return `
            <tr>
              <td class="row-header">${labels[method] || method}</td>
              ${sortedDates.map(d => {
                const wd = getWeekday(d);
                const isWeekend = wd === 'L' || wd === 'P';
                const val = installByDate[d]?.resources[method];
                if (val) {
                  return `<td class="resource-cell crane ${isWeekend ? 'weekend' : ''}">${val}</td>`;
                }
                return `<td class="${isWeekend ? 'weekend' : ''}"><span class="empty">-</span></td>`;
              }).join('')}
            </tr>
        ...
        ${(['troppija', 'monteerija', 'keevitaja'] as InstallMethodType[]).map(method => {
          const labels: Record<string, string> = {
            troppija: '🔗 Troppija',
          ...
          return `
            <tr>
              ...
              ${sortedDates.map(d => {
                ...
                const val = installByDate[d]?.resources[method];
                if (val) {
                  return `<td class="resource-cell labor ${isWeekend ? 'weekend' : ''}">${val}</td>`;
                }
                return `<td class="${isWeekend ? 'weekend' : ''}"><span class="empty">-</span></td>`;
              }).join('')}
            </tr>
  ...
  <p style="margin-top: 16px; color: #888; font-size: 11px;">
    Genereeritud: ${new Date().toLocaleString('et-EE')} |
    Tarned: ${Object.keys(deliveryByFactory).length} tehast |
    Paigaldus: ${scheduleItems.length} detaili
  </p>
</body>
</html>`;
      ...
      if (popup) {
        ...
        setMessage('Gantt graafik avatud');
      } else {
        setMessage('Popup blokeeritud - luba popupid');
      }
    } catch (e) {
      console.error('Error opening Gantt timeline:', e);
      setMessage('Viga Gantt graafiku avamisel');
    }
  };
