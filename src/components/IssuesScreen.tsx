...
export default function IssuesScreen({
  ...
  const selectIssueInModel = useCallback(async (issue: Issue) => {
    if (!issue.objects || issue.objects.length === 0) {
      setMessage(`⚠️ ${t('issues.notLinkedToModel')}`);
      return;
    }
    ...
    try {
      ...
      if (modelObjectIds.length > 0) {
        ...
        setMessage(`⚠️ ${t('issues.objectNotFoundInModel')}`);
      }
  ...
  const selectStatusInModel = useCallback(async (status: IssueStatus) => {
    ...
    if (statusIssues.length === 0) {
      setMessage(`⚠️ ${t('issues.noNonConformancesInStatus')}`);
      return;
    }
    ...
    try {
      ...
      if (modelObjectIds.length > 0) {
        await api.viewer.setSelection({ modelObjectIds }, 'set');
        await api.viewer.setCamera({ selected: true }, { animationTime: 500 });
        setMessage(`✓ ${t('issues.selectedNonConformances', { count: statusIssues.length, label: ISSUE_STATUS_CONFIG[status].label })}`);
      } else {
        setMessage(`⚠️ ${t('issues.objectsNotFoundInModel')}`);
      }
  ...
  const colorStatusInModel = useCallback(async (status: IssueStatus) => {
    ...
    try {
      ...
      setMessage(`✓ ${t('issues.coloredNonConformances', { count: statusIssues.length })}`);
      setColoringStatus('');
  ...
  const handleSubmitIssue = useCallback(async () => {
    ...
    if (!editingIssue && newIssueObjects.length === 0) {
      setMessage(`⚠️ ${t('issues.atLeastOneDetailRequired')}`);
      return;
    }
    ...
    try {
      if (editingIssue) {
        ...
        setMessage(`✅ ${t('issues.nonConformanceUpdated')}`);
        ...
        setMessage(`✅ ${t('issues.nonConformanceCreated')}`);
      }
  ...
  const handleDeleteIssue = useCallback(async (issueId: string) => {
    ...
    try {
      ...
      setMessage(`✅ ${t('issues.nonConformanceDeleted')}`);
      setShowDetail(false);
      setDetailIssue(null);
      await loadIssues();
      await colorModelByIssueStatus();
  ...
  const handleStatusChange = useCallback(async (issueId: string, newStatus: IssueStatus) => {
    try {
      ...
      setMessage(`✅ ${t('issues.statusChanged', { label: ISSUE_STATUS_CONFIG[newStatus].label })}`);
      await loadIssues();
      await colorModelByIssueStatus();
  ...
  const handleAddComment = useCallback(async () => {
    ...
    try {
      ...
      setNewComment('');
      setMessage(`✅ ${t('issues.commentAdded')}`);
  ...
  const handleEditComment = useCallback(async () => {
    ...
    try {
      ...
      setMessage(`✅ ${t('issues.commentUpdated')}`);
  ...
  const handleDeleteComment = useCallback(async (commentId: string) => {
    ...
    try {
      ...
      setMessage(`✅ ${t('issues.commentDeleted')}`);
  ...
  const handleDeleteAttachment = useCallback(async (attachment: IssueAttachment) => {
    ...
    try {
      ...
      setMessage(`✅ ${t('issues.fileDeleted')}`);
  ...
  const handleUnassignUser = useCallback(async (assignmentId: string) => {
    try {
      ...
      setMessage(`✅ ${t('issues.userRemoved')}`);
      await loadIssues();
  ...
  const exportToExcel = useCallback(async () => {
    try {
      ...
      setMessage(`✅ ${t('issues.excelDownloaded')}`);
  ...
  return (
    <div className="issues-screen">
      ...
      <PageHeader
        title={t('issues.nonConformancesTitle', { filtered: filteredIssues.length, total: issues.length })}
        onBack={onBackToMenu}
      ...
      >
        ...
        <button
          ...
          title={currentSelectedObjects.length > 0 ? t('issues.addNonConformanceWithCount', { count: currentSelectedObjects.length }) : t('issues.addNonConformance')}
        >
          <FiPlus size={18} />
          {currentSelectedObjects.length > 0 ? t('issues.addWithCount', { count: currentSelectedObjects.length }) : t('issues.addBtnLabel')}
        </button>
      </PageHeader>
      ...
      {showFilters && (
        <div className="issues-filters">
          ...
          >
            <option value="all">{t('issues.allResponsibles')}</option>
          ...
          <label className="filter-checkbox">
            ...
            />
            <span>{t('issues.overdueFilter')}</span>
          </label>
      ...
      <div className="issues-list">
        {loading ? (
          <div className="issues-loading">
            ...
            <span>{t('issues.loadingNonConformances')}</span>
          </div>
        ) : filteredIssues.length === 0 ? (
          <div className="issues-empty">
            <FiAlertCircle size={48} />
            <p>{t('issues.noNonConformancesFound')}</p>
            <p className="issues-empty-hint">
              {t('issues.selectFromModelHint')}
            </p>
          </div>
        ) : (
                      ...
                      >
                        <div className="issue-card-row">
                          ...
                          {(issue.comments_count || 0) > 0 && (
                            <span className="issue-meta-icon" title={t('issues.commentsCount', { count: issue.comments_count })}>
                              <FiMessageSquare size={10} />
                            </span>
                          )}
                          {(issue.attachments_count || 0) > 0 && (
                            <span className="issue-meta-icon" title={t('issues.photosCount', { count: issue.attachments_count })}>
                              <FiCamera size={10} />
                            </span>
                          )}
                          <span
        ...
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal-content issue-form-modal issue-form-modal-compact" onClick={e => e.stopPropagation()}>
            <div className="modal-header compact">
              <h2>{editingIssue ? t('issues.editNonConformance') : t('issues.addNewNonConformance')}</h2>
              <button onClick={() => {
                setShowForm(false);
                setNewIssueObjects([]);
                setSelectedSubDetailsByParent(new Map());
            ...
            <div className="issue-form issue-form-compact">
              ...
              {!editingIssue && (
                <div className="form-section" style={{ marginBottom: '8px' }}>
                  <label style={{ marginBottom: '2px', fontSize: '11px' }}>
                    {t('issues.columnDetails')} ({(showSubDetailsModal ? lockedParentObjects : newIssueObjects).length})
                  ...
                  {(showSubDetailsModal ? lockedParentObjects : newIssueObjects).length === 0 ? (
                    <div style={{
                      ...
                      {t('issues.selectDetailsWithAssemblySelection')}
                    </div>
                  ) : (
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  <label>{t('issues.discoveryDate')}</label>
                  <input
                    type="date"
              ...
              <div className="form-actions">
                ...
                >
                  {editingIssue ? t('issues.saveChangesBtn') : t('issues.addNonConformance')}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="modal-content issue-detail-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="issue-detail">
              ...
              <div className="detail-info-grid">
                ...
                <div className="info-item">
                  <span className="info-label">{t('issues.columnDetected')}</span>
                ...
                <div className="info-item">
                  <span className="info-label">{t('issues.reporter')}</span>
              ...
              <div className="detail-section">
                ...
                />
                <p className="paste-hint">{t('issues.pasteImageHint')}</p>
              ...
              <div className="detail-section">
                <h4>
                  ...
                  {t('issues.commentsSection')} ({issueComments.length})
                </h4>
                      ...
                      <div key={comment.id} className="comment-item">
                        ...
                        {comment.old_status && comment.new_status && (
                          <div className="comment-status-change">
                            {t('issues.statusLabel')} {ISSUE_STATUS_CONFIG[comment.old_status].label} →{' '}
          ...
          <div className="modal-content assembly-modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '16px' }}>
              <p style={{ marginBottom: '12px', fontSize: '13px', color: '#64748b' }}>
                {t('issues.assemblySelectionDescription')}
              </p>
        ...
        <div className="modal-overlay" onClick={closeSubDetailsModal}>
          <div className="modal-content sub-details-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '360px', display: 'flex', flexDirection: 'column', maxHeight: '80vh' }}>
            <div className="modal-header" style={{ padding: '8px 12px', flexShrink: 0 }}>
              <h3 style={{ fontSize: '13px' }}>{t('issues.subDetailsTitle', { count: subDetails.length })}</h3>
                ...
                {subDetails.map((detail) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {isSelected ? '✓' : t('issues.subDetailLink')}
                      </button>
                    </div>
                  );
                })}
            ...
            <div style={{ padding: '8px 12px', borderTop: '1px solid #e2e8f0', display: 'flex', gap: '6px', justifyContent: 'flex-end', flexShrink: 0, background: 'white' }}>
              ...
              >
                {t('issues.subDetailConfirm', { count: selectedSubDetailsByParent.get(currentSubDetailsParentGuid)?.size || 0 })}
              </button>
            </div>
          </div>
        </div>
