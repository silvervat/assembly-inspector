...
-- Inspection photos gallery view (optional - depends on existing table structure)
-- Using result_id as column name (common naming convention)
DO $$
BEGIN
  CREATE OR REPLACE VIEW v_inspection_photos_gallery AS
  SELECT
    p.id,
    p.result_id as inspection_result_id,
    p.url,
    p.thumbnail_url,
    p.photo_type,
    p.original_filename,
  ...
  JOIN inspection_results r ON r.id = p.result_id
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Could not create v_inspection_photos_gallery view: %', SQLERRM;
END $$;

-- User inspection summary view (optional - depends on existing table structure)
DO $$
BEGIN
  ...
  LEFT JOIN inspection_result_photos p ON p.result_id = r.id
  GROUP BY u.id, u.email, u.name, u.trimble_project_id, u.role, u.inspector_prefix;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Could not create v_user_inspection_summary view: %', SQLERRM;
END $$;
